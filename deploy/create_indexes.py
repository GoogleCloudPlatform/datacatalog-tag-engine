...
def destroy_indexes(project, database):
    indicies_count = 0
    with open(YAML_FILE) as yf:
        full = yaml.full_load(yf)
        indexes = full.get("indexes")
        collections = set(map(lambda idx: idx["collection"], indexes))
        for coll_name in collections:
            parent = f'projects/{project}/databases/{database}/collectionGroups/{coll_name}'
            collection_indices = firestore_client.list_indexes(parent=parent)
            for index in collection_indices:
                try:
                    firestore_client.delete_index(name=index.name)
                    indicies_count += 1
                except Exception as e:
                    print('Error occurred while deleting index', index.name,
                          'on', coll_name, '. Error:', e)

    print('Deleted', indicies_count, 'indexes.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('mode', help='Create or Destroy indices', choices=[
        "create", "destroy"])
    ...
    if args.mode == "create":
        ...
        destroy_indexes(args.firestore_project, args.firestore_database)
    else:
        print('Invalid mode')
